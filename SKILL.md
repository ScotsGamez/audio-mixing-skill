...
5. **In-Place Updates Only (No Preset Sprawl)**: When adjusting or fine-tuning sound profiles, NEVER generate unnecessary extra preset files. Overwrite the user's active preset (`Cloud Alpha S - Kinetic Bass & Crystal Air.json`) in-place so EasyEffects only ever contains this single active preset and the default program presets.
